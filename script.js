document.addEventListener("DOMContentLoaded", function () {
  ...
  function handleGemClick(e) {
      ...
      if (!selectedGem) {
          ...
          if (validMove) {
              swapColors(selectedGem, clickedGem);
              checkMatches();
          } else {
...
// Check if the move is valid
  function isValidMove(gem1, gem2) {
    ...
    // Swap dataset colors for validation
    ...
    // Swap back the colors to their original state
    gem1.dataset.color = tempColor;
    gem2.dataset.color = gem1.dataset.color;

    return validMove;
  }

  // Check for consecutive gems of the same color
  function checkConsecutiveGems(row, col, color, direction) {
    let count = 1;
    let i = row;
    let j = col;

    while (direction === "horizontal" && j > 0 && board.children[i * gridSize + j - 1].dataset.color === color) {
        count++;
        j--;
    }
    while (direction === "vertical" && i > 0 && board.children[(i - 1) * gridSize + j].dataset.color === color) {
        count++;
        i--;
    }
  ...
  // Swap colors between two gems
  function swapColors(gem1, gem2) {
    const gem1Index = Array.from(board.children).indexOf(gem1);
    const gem2Index = Array.from(board.children).indexOf(gem2);

    // Check if gems are adjacent and gem1 would create a match if moved to gem2's position
    const gem1Row = Math.floor(gem1Index / gridSize);
    const gem1Col = gem1Index % gridSize;
    const gem2Row = Math.floor(gem2Index / gridSize);
    const gem2Col = gem2Index % gridSize;
    ...
    const isAdjacent = Math.abs(gem1Row - gem2Row) + Math.abs(gem1Col - gem2Col) === 1;
    const gem1Color = gem1.dataset.color;
    const gem2Color = gem2.dataset.color;

    const horizontalMatch = checkConsecutiveGems(gem1Row, gem1Col, gem1Color, "horizontal") >= 2;
    const verticalMatch = checkConsecutiveGems(gem1Row, gem1Col, gem1Color, "vertical") >= 2;

    if (isAdjacent && (horizontalMatch || verticalMatch)) {
        // Animate the swap
        ...
        // Invalid move animation
        gem1.classList.add("invalid");
        gem2.classList.add("invalid");
        setTimeout(() => {
            gem1.classList.remove("invalid");
            gem2.classList.remove("invalid");
        }, 300);
    }
  }
